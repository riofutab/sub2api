...
func TestOpus55ResponsesAdaptiveThinkingAndToolChoice(t *testing.T) {
	...
	// 合并 #7509 时改此断言：opus-5 族在本仓的档位目录里用 adaptive thinking
	// （见 responses_to_anthropic_reasoning_test.go），上游基线的 enabled 不适用。
	require.Equal(t, "adaptive", old.Thinking.Type)
}
