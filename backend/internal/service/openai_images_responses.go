...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	if parsed.Stream {
		if direct {
			...
			usage, imageCount, imageOutputSizes, firstTokenMs, err = s.handleOpenAIImagesOAuthStreamingResponse(resp, c, startTime, parsed.ResponseFormat, openAIImagesStreamPrefix(parsed), upstreamModel, maskCompositor)
		}
		if err != nil {
			if imageCount > 0 {
